...
import { useMenuChildren } from '@/store/components/menu';
import { MENU_SIDEBAR } from '@/store/config/menu.config';
import { useScrollPosition } from '@/store/hooks/useScrollPosition';
import { useMenus } from '@/core/providers';
import { ILayoutConfig, useLayout } from '@/core/providers';
import { deepMerge } from '@/core/utils';
import { ShopLayoutConfig } from '.';

// Interface defining the structure for layout provider properties
export interface IShopLayoutProviderProps {
...
const initalLayoutProps: IShopLayoutProviderProps = {
  layout: ShopLayoutConfig, // Default layout configuration
...
const ShopLayoutContext = createContext<IShopLayoutProviderProps>(initalLayoutProps);


console.log('ShopLayoutContext : ', ShopLayoutContext);
// Custom hook to access the layout context
const useShopLayout = () => useContext(ShopLayoutContext);

// Layout provider component that wraps the application
const ShopLayoutProvider = ({ children }: PropsWithChildren) => {
  ...
  const getLayoutConfig = () => {
    return deepMerge(ShopLayoutConfig, getLayout(ShopLayoutConfig.name));
  };
  ...
  const setSidebarCollapse = (collapse: boolean) => {
    ...
    updateLayout(ShopLayoutConfig.name, updatedLayout); // Updates the layout with the collapsed state
...
export { ShopLayoutProvider, useShopLayout };
